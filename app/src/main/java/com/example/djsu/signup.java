...
public class signup extends AppCompatActivity {
                ...
                if (UserID.length() >= 5 && UserID.length() <= 20) { // 아이디가 5자 - 20자
                    if (UserPass.length() >= 8 && UserPass.length() <= 16) { // 비밀번호가 8자 - 16자
                        if (UserPass.equals(UserPass2)){ // 비밀번호, 비밀번호 확인이 같은지
                            addUser(et_id.getText().toString(),et_pass.getText().toString(), et_age.getText().toString(), et_name.getText().toString(), profile);
    ...
    public void addUser(String UserId, String UserPass, String UserAge, String UserName, String Profile) {
        ...
        member member = new member(UserId,UserPass,UserAge,UserName,Profile);
